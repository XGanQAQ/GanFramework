...
namespace GanFramework.Editor.Utils
{
    public static class UpdateCheckUtil
    {
        ...
        private const string LastCheckKey = "GanFramework_LastUpdateCheck";
        ...
        private static async Task CheckUpdateAsync(bool isManual)
        {
            ...
            try
            {
                ...
                if (isManual)
                {
                    LogUtil.Info("版本检查", "开始检查 Gan Framework 更新...");
                }
                
                if (!Client.DefaultRequestHeaders.UserAgent.ToString().Contains("Gan-Framework"))
                {
                    Client.DefaultRequestHeaders.UserAgent.ParseAdd("Gan-Framework");
                }
                ...
                if (!string.IsNullOrEmpty(latestVersion) && latestVersion != currentVersion)
                {
                    LogUtil.Warn(
                        "版本更新检查",
                        $"Gan Framework 有新版本！当前：{currentVersion} → 最新：{latestVersion}\n" +
                        $"更新地址：https://github.com/finkkk/Gan-Framework/releases"
                    );
                }
            }
