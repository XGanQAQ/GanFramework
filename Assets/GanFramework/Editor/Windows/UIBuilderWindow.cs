...
namespace GanFramework.Editor.Windows
{
    ...
    public class UIBuilderWindow : EditorWindow
    {
        ...
        [MenuItem("Gan Framework/创建 UI 面板")]
        ...
        private void CreateScript()
        {
            ...
            TextAsset templateAsset = AssetDatabase.LoadAssetAtPath<TextAsset>(
                "Assets/GanFramework/Editor/EditorResources/UI/template_ui_panel.txt"
            );

            if (!templateAsset)
            {
                LogUtil.Error("UIBuilderWindow", "模板文件未找到：Assets/GanFramework/Editor/EditorResources/UI/template_ui_panel.txt");
                return;
            }
        ...
        private string BuildUsings(bool useTMP, bool addBtn, bool addInput, bool addToggle, bool addSlider)
        {
            var us = new HashSet<string> {
                // BasePanel 必须要
                "using GanFramework.Runtime.UI.Base;" };

            // LogUtil 必须要（只要有任何逻辑）
            if (addBtn || addInput || addToggle || addSlider)
                us.Add("using GanFramework.Runtime.Utils;");
        ...
        private static string GetUIPrefabPath(bool useTMP, string prefabName)
        {
            ...
            return $"Assets/GanFramework/Editor/EditorResources/UI/UIBuilder/{typeFolder}/{prefabName}.prefab";
        }
