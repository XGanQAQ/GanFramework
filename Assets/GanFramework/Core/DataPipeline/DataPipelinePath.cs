...
using GanFramework.Core.Settings.Loaders;
using GanFramework.Core.Utils;

namespace GanFramework.Core.DataPipeline
{
    /// <summary>
    /// 数据管线的统一路径访问工具。
    ...
    ///
    /// 读表路径：（始终不变）
    /// Excel 数据源 → GanFramework_Data/DataTables 
    /// 
    /// 外部路径：
    ///   C#     → GanFramework_Data/AutoGen//Data/DataClass/
    ///   JSON   → GanFramework_Data/AutoExport/DataJson （Binary 模式才有此目录；Json 模式自动选择内部路径不导出此目录）
    ///   Binary → GanFramework_Data/AutoExport/DataBinary
    /// 
    /// 内部路径：
    ///   C#     → Assets/Scripts/Data/AutoGen/DataClass
    ///   JSON   → Assets/StreamingAssets/GanFramework_Data/DataJson   (只有Json 模式自动选择内部路径才有此目录)
    ///   Binary → Assets/StreamingAssets/GanFramework_Data/DataBinary
    /// </summary>
    public static class DataPipelinePath
    {
        // 外部数据根目录（C#）
        public static readonly string ExternalCSharpRoot = Path.Combine(ProjectRoot, "GanFramework_Data/AutoGen");
        
        // 外部数据根目录（JSON / Binary 路径）
        public static readonly string ExternalAutoExportRoot = Path.Combine(ProjectRoot, "GanFramework_Data/AutoExport");
        ...
        public static readonly string InternalStreamingRoot = Path.Combine(Application.streamingAssetsPath, "GanFramework_Data");
        ...
        public static string ExcelRoot
        {
            get
            {
                string path = Path.Combine(ProjectRoot, "GanFramework_Data/DataTables");
                PathUtil.EnsureDirectory(path);
                return PathUtil.NormalizePath(path);
            }
        }
